...
def trace_compare(isa_csv, rtl_log, toplevel, strategy=''):
    ...
    try:
        ...
        for j in range(len(isa_csv[isa_idx_offset:])): # skip the first line with headers
            line = isa_csv[isa_idx_offset+j+offset]
            #print(line)
            #if ('ecall') in line:
            ...
            if toplevel=='RocketTile':
                ...
                #dcsr_rtl = rtl_line[6]
            print("----------------------------------------------------")
            print("PC: ",pc_isa, pc_rtl)
            print("ISA: ",instr_str_isa)
            print("RTL: ",instr_rtl)
            print("MODE: ",mode_isa, mode_rtl)
            print("WDATA: ",wdata_isa, wdata_rtl)
            print("MSTATUS: ",mstatus_isa, mstatus_rtl)
            print("FRM: ",frm_isa, frm_rtl)
            print("FFLAGS: ",fflags_isa, fflags_rtl)
            print("MCLAUSE: ",mcause_isa, mcause_rtl)
            print("SCLAUSE: ",scause_isa, scause_rtl)
            print("MEDELEG: ",medeleg_isa, medeleg_rtl)
            if pc_isa!=pc_rtl:
...
def setup(dut, toplevel, template, out, proc_num, debug, minimizing=False, no_guide=False):
    ...
    cc = '/opt/riscv64-unknown-elf-toolchain-10.2.0-2020.12.8-x86_64-linux-ubuntu14/bin/riscv64-unknown-elf-gcc'
