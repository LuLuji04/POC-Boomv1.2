...
class tlAdapter():
    ...
    def drive_input(self, memory):
        ...
        self.a_monitor = cocotb.start_soon(self.a_port_monitor(memory, block_perm, d_sinks, \
                                                         b_srcs, b_callback))
        self.c_monitor = cocotb.start_soon(self.c_port_monitor(memory, block_perm, b_srcs, \
                                                         b_callback))
        self.e_monitor = cocotb.start_soon(self.e_port_monitor(memory, d_sinks))

        self.d_driver = cocotb.start_soon(self.d_port_driver())
        self.b_driver = cocotb.start_soon(self.b_port_driver())

        self.retriever = cocotb.start_soon(self.data_retriever(block_perm, b_srcs, b_callback))
        self.host_if = cocotb.start_soon(self.host_interface(block_perm, b_srcs, b_callback))
