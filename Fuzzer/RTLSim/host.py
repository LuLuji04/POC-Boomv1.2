...
class rvRTLhost():
    ...
    def run_test(self, rtl_input: rtlInput, assert_intr: bool, iteration):
        ...
        clk = self.dut.clock
        clk_driver = cocotb.start_soon(self.clock_gen(clk))
        clkedge = RisingEdge(clk)
        self.dut.iteration.value = iteration
        yield self.reset(clk, self.dut.metaReset, self.dut.reset)
        self.dut.eos.value = 0
        ...
        for i in range(max_cycles):
            ...
            if i % 100 == 0:
                ...
                if tohost:
                    self.dut.eos.value = 1  # chath: set end of sim
                    break
                else:
                    self.adapter.probe_tohost(tohost_addr)

        self.dut.eos.value = 1
